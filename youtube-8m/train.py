...
if __name__ == "__main__":
  ...
  flags.DEFINE_integer(
      "num_epochs", None, "How many passes to make over the dataset before "#5
      "halting training.")
  flags.DEFINE_integer(
      "max_steps", None,
  ...
  flags.DEFINE_integer(
      ...
      "is exported for batch prediction.")#1000
...
def build_graph(reader,
  ...
  whitelisted_cls_mask=whitelisted_cls_mask*4+np.ones((3862,),dtype=np.float32)
  ...
  for i in range(num_towers):
    ...
    with tf.device(device_string % i):
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):# reuse=True if i > 0 else None
        with (slim.arg_scope([slim.model_variable, slim.variable],
          ...
          if "loss" in result.keys():
            ...
            label_loss = label_loss_fn.calculate_loss(predictions,
                                                      tower_labels[i],label_weights=whitelisted_cls_mask)
            if "aux_predictions" in result.keys():
                for pred in result["aux_predictions"]:
                    label_loss += label_loss_fn.calculate_loss(pred, tower_labels[i],label_weights=whitelisted_cls_mask)
...
#                           task_as_string(self.task))
  ...
  def build_model(self, model, reader):
    ...
    return tf.train.Saver(max_to_keep=0, keep_checkpoint_every_n_hours=0.5)#0.25
