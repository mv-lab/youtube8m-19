...
if __name__ == "__main__":
  ...
  flags.DEFINE_integer("segment_max_pred", 100000,#25000
...
def get_input_data_tensors(reader, data_pattern, batch_size, num_readers=1):
  ...
  with tf.name_scope("input"):
    ...
    print('examples_and_labels',examples_and_labels)
  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    if FLAGS.segment_labels:
      ...
      if FLAGS.segment_label_ids_file:
        whitelisted_cls_mask = np.zeros((predictions_tensor.get_shape()[-1],),
                                        dtype=np.float32)
        # from tensorflow.python.lib.io import file_io
        # with file_io.FileIO(FLAGS.segment_label_ids_file, mode ='r') as fobj:
        import csv
        import urllib2
    ...
    try:
      while not coord.should_stop():
        ...
        if FLAGS.segment_labels:
          # print('video_batch_val',video_batch_val[:,15:,:].shape)
          # print('num_frames_batch_val',num_frames_batch_val-15)
          # results = get_segments(video_batch_val, num_frames_batch_val, 5)
          results = get_segments(video_batch_val[:,30:,:], num_frames_batch_val-30-30, 5)#15
          results["video_segment_ids"][:,1]=results["video_segment_ids"][:,1]+30
        ...
        # print('video_batch_val',video_batch_val.shape)
      ...
      if FLAGS.segment_labels:
        ...
        for line in out_file:
          ...
          for cls, score in zip(pred_cls_ids, pred_cls_scores):
            ...
            if cls not in heaps:
              heaps[cls] = []
            # if score < 0.05: # minimum probablity to write
            #   continue
